...
```
========================================
  Phantom – Claude Ethical RedTeam
  Installer v1.2.0
========================================
...
The installer handles everything: API key (validated), scope (placeholder rejected), system dependencies, Python packages, and external tools (bettercap, zphisher, CyberStrikeAI).
...
API key is stored in `.env` (generated by the installer) and must be exported before launch:
...
To change the target between engagements, re-run `./install.sh` or edit `scopes/current_scope.md` directly.
